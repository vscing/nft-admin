  ...
  <PageWrapper title="基础表单" contentBackground contentClass="p-4">
    <Form ref="formRef" :model="objData.formData" :label-col="labelCol" :wrapper-col="wrapperCol" :rules="rules">
      <FormItem label="轮播图标题" name="name">
        <Input v-model:value="objData.formData.name" placeholder="请输入" />
      </FormItem>
...
import { getBannerInfo, updateBannerInfo } from '/@/api/sys/content';
...
const onSubmit = () => {
  formRef.value
    .validate()
    .then(async () => {
      const res = await updateBannerInfo({...objData.formData, id: id, presell_time: formatToDateTime(objData.formData.presell_time)});
      if(res) {
